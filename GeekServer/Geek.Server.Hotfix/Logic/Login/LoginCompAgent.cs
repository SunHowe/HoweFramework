...
namespace Server.Logic.Logic.Login
{
    public class LoginCompAgent : StateCompAgent<LoginComp, LoginState>
    {
        ...
        public async Task OnLogin(NetChannel channel, LoginReq reqLogin, LoginResp resp)
        {
            if (string.IsNullOrEmpty(reqLogin.UserName))
            {
                throw new ServerErrorCodeException(ServerErrorCode.Login_AccountCannotBeNull);
            }

            if (reqLogin.Platform != "android" && reqLogin.Platform != "ios" && reqLogin.Platform != "unity")
            {
                //验证平台合法性
                throw new ServerErrorCodeException(ServerErrorCode.Login_UnknownPlatform);
            }
            ...
            await roleComp.OnLogin(isNewRole, resp);
