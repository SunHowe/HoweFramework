...
namespace MessagePackCompiler.CodeGenerator.CS
{
    public class InnerGenerator
    {
        ...
        public static InnerGenerator Singleton = new InnerGenerator();
        //sub - parent
        ...
        public void GenCode(Compilation compilation, INamedTypeSymbol[] targetTypes, string output, string clientOutput)
        {
            ...
            foreach (var type in targetTypes)
            {
                ...
                while (baseType != null)
                {
                    if (baseType.ToString().Equals(BaseMessage))
                    {
                        isMsg = true;
                        break;
                    }
            ...
            var msgTemp = Scriban.Template.Parse(File.ReadAllText("Liquid/CS/MsgFactory.liquid"));
            ...
            var registerTemp = Scriban.Template.Parse(File.ReadAllText("Liquid/CS/Register.liquid"));
            ...
            var enumTemp = Scriban.Template.Parse(File.ReadAllText("Liquid/CS/Enum.liquid"));
            ...
            var template = Scriban.Template.Parse(File.ReadAllText("Liquid/CS/Proto.liquid"));
