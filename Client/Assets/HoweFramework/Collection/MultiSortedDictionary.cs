...
namespace HoweFramework
{
    ...
    public class MultiSortedDictionary<TKey, TValue> : IEnumerable<KeyValuePair<TKey, LinkedListRange<TValue>>>, IEnumerable
        where TValue : IComparable<TValue>
    {
        ...
        public void Add(TKey key, TValue value)
        {
            ...
            if (m_Dictionary.TryGetValue(key, out range))
            {
                ...
                var terminal = range.Terminal;
                var node = terminal.Previous;
                while (node != first && node.Value.CompareTo(value) > 0)
                {
                    node = node.Previous;
                }
                ...
                {
                    m_LinkedList.AddAfter(node, value);
                }
            }
            else
            {
        ...
        /// <param name="key">指定的主键。</param>
        /// <param name="value">指定的值。</param>
        /// <param name="removedValue">移除的值。</param>
        /// <returns>是否移除成功。</returns>
        public bool Remove(TKey key, TValue value, out TValue removedValue)
        {
